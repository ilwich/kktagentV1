...
def secret_modify(setting_str_key, setting_str_val):
    """Изменение строчки с параметром в файле скрытых настроек"""
    try:
        with open('C:\\ProgramData\\V_element\\kktagent\\.secrets.toml', 'r') as setting_file:
            setting_data = setting_file.readlines()
            res = []
            for line in setting_data:
                if line.find(setting_str_key.upper()) == 0:
    ...
    if res:
        try:
            with open('C:\\ProgramData\\V_element\\kktagent\\.secrets.toml', 'w') as setting_file:
                for el in res:
                    setting_file.write(el)
        except:
            return False
